...
export function useAppData() {
  ...
  const toggleExercise = useCallback(
    async (workoutId: string, exerciseId: string) => {
      ...
      const updated: TrainingLog = { ...existing, completedExercises: completed, updatedAt: new Date().toISOString() };
  ...
  const saveExerciseLog = useCallback(
    async (
      ...
      const exerciseLog = buildExerciseLog(exerciseId, exerciseName, sets, logs, today, workoutId);
      const existing: TrainingLog =
        idx >= 0
          ? { ...logs[idx], exerciseLogs: [...(logs[idx].exerciseLogs ?? [])] }
          : { date: today, workoutId, completedExercises: [], exerciseLogs: [] };
      ...
      await persist({ ...data, trainingLogs: nextLogs });
      return reconcileTrainingLogPRs(nextLogs)
  ...
  const sendChat = useCallback(
    async (message: string) => {
      ...
      const withUser = { ...data, chatHistory: [...data.chatHistory, userMsg] };
      setData(withUser);

      const reply = await askMari(message, withUser);
      const assistantMsg = createChatMessage("assistant", reply);
      await persist({ ...withUser, chatHistory: [...withUser.chatHistory, assistantMsg] });
    },
    [data, persist]
  );
  ...
  const getLatestWeight = useCallback((): WeightEntry | undefined => {
    ...
    return [...data.weights].sort((a, b) => b.date.localeCompare(a.date))[0];
  }, [data]);
